...
def create_team_members(data: dict, mapping: list, wbi: WikibaseIntegrator, res: list) -> dict:
    """
...
def main() -> None:
    ...
    if isinstance(__data, dict):
        # only one item
        __res.append(create_team_members(__data, column_mapping, wbi, __res))
    elif isinstance(__data, list):
        for item in __data:
            __res.append(create_team_members(item, column_mapping, wbi, __res))
    else:
