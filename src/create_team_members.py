...
def create_team_member(data: dict, mapping: list, wbi: WikibaseIntegrator, res: list) -> dict:
    """
...
def main() -> None:
    ...
    if isinstance(__data, dict):
        ...
        for __key in __keys:
            __res.append(create_team_member(dict({ "{}".format(__key): __data[__key] }), column_mapping, wbi, __res))
